...
def get_live_data():
    global BTC_PRICE
    # 1. Price from Coingecko - always works
    try:
    ...
    bids, asks = [], []
    # 2. Orderbook from Bybit - Render pe block nahi hota
    try:
...
def make_heatmap():
    ...
    if bids:
        ...
        if bx_f:
            ...
            ax.scatter(bx_f, np.random.normal(1, 0.05, len(bx_f)), s=sizes, c='#00FF88', alpha=0.7, edgecolors='none')
    ...
    if asks:
        ...
        if axx_f:
            sizes = np.array(aq_f)
            sizes = 20 + (sizes / max(sizes) * 300)
            ax.scatter(axx_f, np.random.normal(1, 0.05, len(axx_f)), s=sizes, c='#FF4444', alpha=0.7, edgecolors='none')

    ax.axvline(price, color='yellow', linestyle='--', lw=2, label=f'BTC: ${price:,.2f}')
    ax.set_xlim(price-2000, price+2000)
    ax.set_ylim(0.7, 1.3)
    ax.set_yticks([])
    ax.tick_params(colors='white', labelsize=8)
    ax.set_xlabel('BTC Price - Liquidation Levels', color='gray')
    ...
    plt.title("🔥 Lucky Heatmap - Coinglass Style LIVE", color='white', fontsize=12)
    plt.tight_layout()
...
def telegram_loop():
    ...
    while True:
        try:
            ...
            data = {'chat_id': CHAT_ID, 'caption': f'🔥 BTC: ${price:,.2f} - Coinglass Live'}
...
def home():
    ...
    LATEST_IMG = img_b64
    return f"""
    <html><head><meta http-equiv="refresh" content="15"><title>Lucky Heatmap</title></head>
    <body style="background:black;color:white;text-align:center;font-family:Arial;padding:10px;margin:0">
    <h2 style="margin:5px">🔥 Lucky Heatmap - LIVE</h2>
    <h3 style="color:yellow;margin:5px">BTC: ${price:,.2f}</h3>
    <img src="data:image/png;base64,{img_b64}" style="width:98%;max-width:1000px;border-radius:12px;border:1px solid #222">
    <p style="color:#666;font-size:12px">Green=Bids (Long Liq) | Red=Asks (Short Liq) | Auto 15 sec | Telegram 5 min</p>
    </body></html>
    """
